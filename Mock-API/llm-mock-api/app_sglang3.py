...
def update_metrics():
    while True:
        if DYNAMIC_UPDATE_ENABLED:
            metrics["num_requests_waiting"] = round(random.uniform(1000.0, 1800.0), 1)
            metrics["gpu_cache_usage_perc"] = round(random.uniform(0.7, 1.0), 2)
...
@app_sglang3.post("/toggle_update/{state}")
...
if __name__ == "__main__":
    uvicorn.run("app_sglang3:app_sglang3", host="0.0.0.0", port=8015, reload=True)
